	Luis
>     Proposed the site's brand name and base color for the palette
>     Initialized GitHub Pages for the repository and created a dedicated branch
      for the hosted files (we're now using GitHub Actions instead to deploy the
	  site)
>     Deleted unnecessary project files coming from the initial HTML template
>     Fixed the header and footer inconsistencies across the different HTML
      pages
o	Admin Page
>     Created the admin login page
>     Created the main admin dashboard showing the most recent orders, most
      important items, and most recent messages
>     Created the active orders view
>     Created the inventory view showing the main details and amount in stock of
      the items sold through the site
>     Created the messages view showing the most recent messages sent by
      customers through the Contact form, as well as links to reply via email or
	  phone, and a button to delete the message
>     Created the users view
>     Defined the CSS stylesheets for each admin page (except for login, Marcus
      helped with that one)
o	Crud
>     Created the edit order view showing a full report of a specific active
      order and actions that can be taken
>     Created the edit item view showing a full and editable report of a
      specific item, as well as actions that can be taken with it
>     Created the edit user view showing a full and editable report of a
      specific user, as well as actions that can be taken with them
>     Created the new item (create item) view
>     Created the new user (create user) view
>     Defined the CSS stylesheets for each CRUD page
